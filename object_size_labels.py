...
for idx, c in enumerate(cnts):
    ...
    # cv2.circle(orig, (int(tlblX), int(tlblY)), 5, (0, 0, 0), -1)
    # cv2.circle(orig, (int(trbrX), int(trbrY)), 5, (0, 0, 0), -1)
    # cv2.circle(orig, (int(cX), int(cY)), 25, (0, 200, 0), -1)
    #
    ...
    #             0.65, (100, 100, 100), 2)
    mussle = {'count' : idx, 'dimA': dimA, 'dimB': dimB, 'x': int(tltrX), 'y':int(tltrY), 'cX': cX, 'cY': cY}
...
for m in mussles:
    cv2.putText(orig, "{:.1f}in".format(m['dimA']),(m['cX']-15, m['cY']), cv2.FONT_HERSHEY_SIMPLEX,0.5, (255, 255, 255), 2)

cv2.imshow('Image', orig)
cv2.waitKey()

#Initiate CSV file and Writer
with open('mycsv.csv', 'w', newline='') as f:
    thewriter = csv.writer(f)
    thewriter.writerow(['Count', 'dimA', 'dimB'])

    for m in mussles:
        thewriter.writerow([m['count'],m['dimA'],m['dimB']])

    print('Data writen to CSV file succesfully')
